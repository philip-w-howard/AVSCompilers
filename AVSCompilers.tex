...
\item Recursive Rules of the form, ``If $x$ is in L, the so is $f(x)$''. Rules 2a-2e for AE are the recursive rules. If the language is a set of strings, then the recursive rules always extend or combine strings. That is, they always make longer strings, they never remove characters from the string. If the language is a set of strings, the recursive rules must specify string operations not numeric operations. The language EVEN was a set of numbers, not strings; as a result, the recursive rule was allowed to perform arithmetic operations.
\end{itemize}
