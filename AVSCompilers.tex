...
Consider the following non-recursive definition of POLYNOMIAL:
\begin{enumerate}
\item All numbers are in POLYNOMIAL
\item The variable $x$ is in POLYNOMIAL
\item The operators $+ - * /$ can be used in POLYNOMIAL
\item POLYNOMIAL does not allow two numbers in a row
\item POLYNOMIAL does not allow two operators in a row
\item POLYNOMIAL does not allow a leading or trailing operator
\item POLYNOMIAL can use parenthesis to group things
\end{enumerate}

Is this list of rules sufficient? Does it allow all valid polynomials and does it rule out all invalid polynomials? It turns out, the answer to some of these questions is ``no'' ($-x$ is a valid polynomial but it is disallowed by Rule 6). We could try to extend and modify the list of rules, but this would, at best, be cumbersome. Let's try a recursive definition:
...
\item the variable $x$ is in POLYNOMIAL
\item if $p$ and $q$ are in POLYNOMIAL, then so are:
\begin{enumerate}
\item $p+q$
\item $p-q$
\item $(p)$
...
From this definition, could we argue that $5x^3 + 7X^2 + 3x + 7$ is in POLYNOMIAL? Almost. This notation includes a mathematical short-cut that is not included in our definition. It includes a superscripted number (e.g. $x^3$) to mean repeated multiplication. If we allow that short-cut, the proof can follow the following pattern:
\begin{enumerate}
\item 5 is in POLYNOMIAL (Rule 1)
\item $x$ is in POLYNOMIAL (Rule 2)
\item $5x$ is in POLYNOMIAL (Lines 1 and 2 and Rule~3d)
\item $5x^2$ is in POLYNOMIAL (Lines 2 and 3 and Rule~3d)
\item \ldots
\end{enumerate}
It should be obvious how to extend this to complete the proof.
zzz
